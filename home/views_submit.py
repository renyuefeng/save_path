...
def get_files(request):
    ...
    if upload_file:
        for file in upload_file:
            ...
            #
            # start_time = time.time()
            newdoc = PurchaseContractTime(file_path=file, contract_sort='采购合同', file_name=str(file))
            newdoc.save()
            # end_time = time.time()
            # print(start_time - end_time, '第一次存表: %.6f' % (end_time - start_time))

            # start_time = time.time()
            # if len(upload_file) > 1:

            # else:
            dic['fid'] = newdoc.id
            file_name = PurchaseContractTime.objects.get(id=newdoc.id).file_path
            # 读数据
            strings = readDataFromPDF(file)
...
def show_file_message(request):
    ...
    # if PurchaseContractTime.objects.get(id=fid):
        ...
        # print(main_constract.purchase_contract.time_of_signing)
        # print(type(main_constract.purchase_contract.time_of_signing))
        try:
            ...
            # if main_constract.remark:
            #     info['data']['remark'] = main_constract.remark
            # if main_constract.purchase_contract.time_of_signing:
            #     info['data']['time_of_signing'] = main_constract.purchase_contract.time_of_signing.strftime('%Y-%m-%d')
        except:
            pass
    ...
    # print(info['data']['time_of_signing'])
    if info['data']['time_of_signing']:
        info['data']['time_of_signing'] = info['data']['time_of_signing'].strftime('%Y-%m-%d')
    # print(info['data']['time_of_signing'].strftime('%Y-%m-%d'))
    # print('info', info)
