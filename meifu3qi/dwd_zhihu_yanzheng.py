...
df = a.read_database(table_name='dwd_zhihu_main')
df1 = a.read_database(table_name='dwd_zhihu_publisher', list_field=['attention_count', 'publisher_nikename'])
pattern = re.compile(
...
df['interactive_count'] = list(map(lambda x, y, z, a, b: x+0.001*y+z+a+b, df['attention_count'], df['click_count'], df['like_count'], df['publish_comment_count'], df['comment_count']))
df2 = df1.sort_values(by=['attention_count'], ascending=False).head(math.ceil(df1.shape[0]/5))[['publisher_nikename']]
df2['user_sort'] = 'PGC'
df1 = pd.merge(df1, df2, on='publisher_nikename', how='left')
pattern_g = re.compile('官方')
df1['user_sort'] = list(map(lambda x, y: 'BGC' if pd.notna(y) and pattern_g.search(y)!=None else(x if pd.notna(x) else 'UGC'), df1['user_sort'], df1['publisher_nikename']))
df1.drop(['attention_count'], axis=1, inplace=True)
df = pd.merge(df, df1, left_on='answer_nickname', right_on='publisher_nikename', how='left')
...
df.drop(['flag', 'publisher_nikename'], axis=1, inplace=True)
...
a = sql_test.dbconnect(host, username, password, database, port)
a.insert_database(table_name='dwd_zhihu_main_keywordMerge', data=df, if_exists='truncate')
