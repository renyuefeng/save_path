...
df = a.read_database(table_name='dim_海量3期_copy')
# df1 = a.read_database(table_name='dwd_douyin_publisher', list_field=['publisher_nickname', 'fans_count', 'auth'])
pattern = re.compile(
    '嘉实多EDGE|EDGE极护|嘉实多 EDGE|EDGE 极护|NBA总决赛|昆仑机油|勇士总冠军|昆仑润滑油|LPL2022|NBA突破时刻|长城机油|LPL夏季赛|Discovery探索频道|英超联赛2022|美孚1号|英雄联盟职业联赛|克莱汤普森|KlayThompson|英雄联盟赛事|统一机油|王者荣耀职业联赛|嘉实多|统一润滑油|KPL夏季赛|长城润滑油|美孚1号NBA|LPL深夜档|美孚NBA|恒久动力，驰骋天地|嘉实多磁护|无畏竞巅峰|美孚速霸|美孚车用润滑油|突破无止境|拯救者 LPL|嘉实多极护|嘉实多magnatec|嘉实多2021/22英超关键先生|壳牌先锋|KPL王者荣耀职业联赛|嘉实多2021/22英超赛季最佳|嘉实多Castrol官方微博|萧敬腾 LPL|为爱而聚 E起前进|2022KPL夏季赛|美孚1号突破礼盒|关键时刻燃擎护航|壳牌锐净|探索未知世界，寻找自然奇迹；挑战已知界限，释放无限潜力|壳牌|美孚|LPL|lpl|kpl|Kpl|KPl|Lpl|龙蟠机油|龙蟠润滑油|EDG|edg|Edg|2022 KPL|决赛|NBA|Nba|nba|道达尔|KPL|英雄联盟|英超|Discovery|discovery|DISCOVERY|王者荣耀|勇士|冠军|磁护|极护|超凡喜力|克莱|美孚一号'
)
...
df['keyword'] = list(map(lambda x: re.compile('\W*(EDG)(?=\W|$)|\W*(edg)(?=\W|$)|\W*(Edg)(?=\W|$)').sub('', x) if pd.notna(x) and re.compile('\W*(EDG)(?=\W|$)|\W*(edg)(?=\W|$)|\W*(Edg)(?=\W|$)').search(x) != None and re.compile('2022 KPL|KPL|王者荣耀职业联赛|KPL夏季赛|KPL王者荣耀职业联赛|2022KPL夏季赛|kpl|王者荣耀|Kpl').search(x) !=None else x, df['keyword']))
...
if pd.notna(x) and pattern_m.search(x) != None else('嘉实多,英超' if pd.notna(x) and pattern_j.search(x) != None
                                                    else('壳牌,F1' if pd.notna(x) and pattern_q.search(x) != None else np.nan)), df['keyword']))
try:
...
df['activity'] = list(map(lambda x, y: y if pd.notna(y) else('LPL' if pattern_1.search(x) != None
                                                              else('NBA' if pattern_2.search(x) != None
                                                                   else('KPL' if pattern_3.search(x) != None
                                                                                                         ...
                                                                                                         else(pattern_9.search(x).group() if pattern_9.search(x) != None else np.nan))))))))))), df['keyword'], df['activity']))

pattern_m1 = re.compile('美孚1号|美孚1号突破礼盒|美孚一号')
...
# df.drop_duplicates(subset=['activity', 'content', 'tag', 'publisher'], inplace=True)
df.drop_duplicates(subset=['activity', 'url_link', 'keyword'], inplace=True)

# pattern_d = re.compile('感染|埃里克|驱蚊|蚊香液|违章停车|蚊子|孕妇|儿童|避蚊胺|轨迹|阳性')
# pattern_m = re.compile('香港|惠州|美孚新村|皇家美孚|公寓|老鼠|茶餐廳|九华径|美孚站|唐美孚|洋行|美孚教育|海南省博物馆|根斯堡|麦当劳|科伦|咖啡|西乡|阳性|新冠|马斯克|美股|美媒|道指|石油|拜登|新邨|环球时报|华尔街日报|乌克兰|美国|俄|警方|A股|股票|美元|股权|福布斯|股东|区块链| Odoptu OP-11油井|美孚酱|美孚英语|gregmat|多多饼店|港独|袭警|西部世界|飞马文学奖')
...
# df['flag'] = list(map(lambda x, y: 0 if x == '美孚' and pattern_m.search(y)!=None else(0 if x == '道达尔' and pattern_d.search(y)!=None
#                                                                                      else(0 if x == '嘉实多' and pattern_j.search(y)!=None
#                                                                                           else(0 if x == '壳牌' and pattern_q.search(y)!=None else 1))), df['activity'], df['tag']))
# df = df.loc[df['flag'] == 1]

df['forward_count'] = list(map(lambda x: x if pd.notna(x) else 0, df['forward_count']))
df['comment_count'] = list(map(lambda x: x if pd.notna(x) else 0, df['comment_count']))
df['like_count'] = list(map(lambda x: x if pd.notna(x) else 0, df['like_count']))
df['click_count'] = list(map(lambda x: x if pd.notna(x) else 0, df['click_count']))

df['interactive_count'] = list(map(lambda x, y, z, a: x+y+z+0.001*a, df['forward_count'], df['comment_count'], df['like_count'], df['click_count']))

# df1['publisher_nickname'] = list(map(lambda x: x.strip(' ') if pd.notna(x) else x, df1['publisher_nickname']))
#
# df2 = df1.sort_values(by=['fans_count'], ascending=False).head(math.ceil(df1.shape[0]/5))[['publisher_nickname']]
# df2['user_sort'] = 'PGC'
# df1 = pd.merge(df1, df2, on='publisher_nickname', how='left')
# pattern_g = re.compile('官方')
# df1['user_sort'] = list(map(lambda x, y: 'BGC' if pd.notna(y) and pattern_g.search(y)!=None else(x if pd.notna(x) else 'UGC'), df1['user_sort'], df1['auth']))
# df1.drop(['fans_count', 'auth'], axis=1, inplace=True)
# df = pd.merge(df, df1, on='publisher_nickname', how='left')
...
# sql = "delete from dwd_weibo_main_keywordMerge where activity='道达尔' and content REGEXP ('感染|埃里克|驱蚊|蚊香液|违章停车|蚊子|孕妇|儿童|避蚊胺|轨迹|阳性')"
# sql1 = "delete from dwd_weibo_main_keywordMerge where activity='美孚' and content REGEXP ('香港|惠州|美孚新村|皇家美孚|公寓|老鼠|茶餐廳|九华径|美孚站|唐美孚|洋行|美孚教育|海南省博物馆|根斯堡|麦当劳|科伦|咖啡|西乡|阳性|新冠|马斯克|美股|美媒|道指|石油|拜登|新邨|环球时报|华尔街日报|乌克兰|美国|俄|警方|A股|股票|美元|股权|福布斯|股东|区块链| Odoptu OP-11油井|美孚酱|美孚英语|gregmat|多多饼店|港独|袭警|西部世界|飞马文学奖')"
# sql2 = "delete from dwd_weibo_main_keywordMerge where activity='嘉实多' and content REGEXP ('肠粉|阳性|新冠|马斯克|美股|美媒|道指|拜登|环球时报|华尔街日报|乌克兰|美国|俄|警方|A股|股票|美元|股权|福布斯|股东|区块链')"
# sql3 = "delete from dwd_weibo_main_keywordMerge where activity='壳牌' and content REGEXP ('大壳牌怒音|阳性|新冠|马斯克|美股|美媒|道指|拜登|环球时报|华尔街日报|乌克兰|美国|俄|警方|A股|股票|美元|股权|福布斯|股东|区块链')"

# a = sql_test.dbconnect(host, username, password, database, port)
# a.read_other_sql(sql=sql)
# a = sql_test.dbconnect(host, username, password, database, port)
# a.read_other_sql(sql=sql1)
# a = sql_test.dbconnect(host, username, password, database, port)
# a.read_other_sql(sql=sql2)
# a = sql_test.dbconnect(host, username, password, database, port)
# a.read_other_sql(sql=sql3)
a = sql_test.dbconnect(host, username, password, database, port)
a.insert_database(table_name='dim_海量3期_copy', data=df, if_exists='truncate')
