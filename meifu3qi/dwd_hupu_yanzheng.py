...
df = a.read_database(table_name='dwd_hupu_main')
df1 = a.read_database(table_name='dwd_hupu_publisher', list_field=['publisher_link', 'level'])
pattern = re.compile(
...
if pd.notna(x) and pattern_m.search(x) != None else('嘉实多,英超' if pd.notna(x) and pattern_j.search(x) != None
                                                    else('壳牌,F1' if pd.notna(x) and pattern_q.search(x) != None else '')), df['keyword']))
print(df.info())
...
pattern_r = re.compile('傲视群雄|登峰造极|一代宗师')
df1['user_sort'] = list(map(lambda x: 'PGC' if pd.notna(x) and pattern.search(x)!=None else 'UGC', df1['level']))
# df1 = df1.sort_values(by=['fans_count'], ascending=False).head(math.ceil(df1.shape[0]/5))
# df1['user_sort'] = 'PGC'
...
df = pd.merge(df, df1, on='publisher_link')
...
a = sql_test.dbconnect(host, username, password, database, port)
a.insert_database(table_name='dwd_hupu_main_keywordMerge', data=df, if_exists='truncate')
