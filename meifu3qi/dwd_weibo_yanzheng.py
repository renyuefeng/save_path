...
df = a.read_database(table_name='dwd_weibo_main')
df1 = a.read_database(table_name='dwd_weibo_publisher', list_field=['pub_id', 'fans_count', 'identity'])
pattern = re.compile(
...
df['interactive_count'] = list(map(lambda x, y, z: x+y+z, df['forward_count'], df['like_count'], df['comment_count']))
df2 = df1.sort_values(by=['fans_count'], ascending=False).head(math.ceil(df1.shape[0]/5))[['pub_id']]
df2['user_sort'] = 'PGC'
df1 = pd.merge(df1, df2, on='pub_id', how='left')
pattern_g = re.compile('官方')
df1['user_sort'] = list(map(lambda x, y: 'BGC' if pd.notna(y) and pattern_g.search(y)!=None else(x if pd.notna(x) else 'UGC'), df1['user_sort'], df1['identity']))
df1.drop(['fans_count', 'identity'], axis=1, inplace=True)
df = pd.merge(df, df1, on='pub_id', how='left')
...
a = sql_test.dbconnect(host, username, password, database, port)
a.insert_database(table_name='dwd_weibo_main_keywordMerge', data=df, if_exists='truncate')
