...
df2 = a.read_database(table_name='db_e_commerce_data_chuimei', list_field=['brand', 'url_link', 'title', 'content', 'pub_time', 'publisher', 'comment_count'])
...
# a = sql_test.dbconnect(host, username, password, database, port)
# df = a.read_database(table_name='dim_summery_comment', list_field=['keyword', 'url_link', 'title', 'content', 'pub_time', 'tag', 'publisher', 'comment_count'])
# df['platform'] = '3q'
...
# # print(url_list)
# # print(ch_list)
sql1 = 'select url_link, replier, comment_content, comment_time, sentiment_analysis, sentiment_flag, purchase_intention  from dim_summery_comment where comment_content REGEXP "0w.{1}20"'
# sql1 = 'select c_url, author, title, release_date  from dwd_t_comment_3nd_quarter where c_url in ({})'.format(','.join(url_list))
sql2 = 'select url_link, replier, comment_content, comment_time from db_e_commerce_data_vm_comment where comment_content REGEXP "0w.{1}20"'
sql3 = 'select Product_SKU, Nickname, Comment, Comment_time from db_e_commerce_data where comment REGEXP "0w.{1}20"'
a = sql_test.dbconnect(host, username, password, database, port)
df = a.read_select_sql(sql1)
df['platform'] = '新媒体'
...
database = 'dim_summery'
a = sql_test.dbconnect(host, username, password, database, port)
a.insert_database(table_name='dim_meifu_0w20_comment_bak_1202', data=df, if_exists='truncate')
