...
df = a.read_database(table_name='dwd_bilibili_main')
df1 = a.read_database(table_name='dwd_bilibili_publihser', list_field=['publisher_id', 'fans_count', 'bilibili_auth'])
pattern = re.compile(
...
df['keyword'] = list(
    map(lambda x, y, z: ','.join(list(set(pattern.findall(x + y + z)))) if pd.notna(str(x) + str(y) + str(z)) and pattern.search(
        str(x) + str(y) + str(z)) != None else np.nan, df['content'],
        df['tag'], df['title']))
...
df['flag'] = list(map(lambda x, y: 0 if x == '美孚' and pd.notna(y) and pattern_m.search(y)!=None else(0 if x == '道达尔' and pd.notna(y) and pattern_d.search(y)!=None
                                                                                     else(0 if x == '嘉实多' and pd.notna(y) and pattern_j.search(y)!=None
                                                                                          else(0 if x == '壳牌' and pd.notna(y) and pattern_q.search(y)!=None else 1))), df['activity'], df['content']))
df = df.loc[df['flag'] == 1]
...
df2 = df1.sort_values(by=['fans_count'], ascending=False).head(math.ceil(df1.shape[0]/5))[['publisher_id']]
df2['user_sort'] = 'PGC'
df1 = pd.merge(df1, df2, on='publisher_id', how='left')
pattern_g = re.compile('官方')
df1['user_sort'] = list(map(lambda x, y: 'BGC' if pd.notna(y) and pattern_g.search(y)!=None else(x if pd.notna(x) else 'UGC'), df1['user_sort'], df1['bilibili_auth']))
df1.drop(['fans_count', 'bilibili_auth'], axis=1, inplace=True)
...
a = sql_test.dbconnect(host, username, password, database, port)
a.insert_database(table_name='dwd_bilibili_main_keywordMerge', data=df, if_exists='truncate')
