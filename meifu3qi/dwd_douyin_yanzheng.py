...
df = a.read_database(table_name='dwd_douyin_main')
...
df['keyword'] = list(
    map(lambda x, y: ','.join(list(set(pattern.findall(x + y)))) if pd.notna(x) and pd.notna(y) and pattern.search(
        ...
        ','.join(list(set(pattern.findall(y)))) if pd.isna(x) and pd.notna(y) else np.nan)), df['tag'],
        df['title']))
...
df['interactive_count'] = list(map(lambda x, y, z, a, b: x+y+z+b+a, df['like_count'], df['comment_count'], df['forward_count'], df['favourite_count'], df['download_count']))
...
a = sql_test.dbconnect(host, username, password, database, port)
a.insert_database(table_name='dwd_douyin_main_keywordMerge', data=df, if_exists='truncate')
